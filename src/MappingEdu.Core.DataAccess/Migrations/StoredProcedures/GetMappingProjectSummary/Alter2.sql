...
WITH project as (
	...
	result AS (
        ...
        JOIN project as mp on MappedSystemId = mp.SourceDataStandardMappedSystemId
            WHERE ((@ParentSystemItemId = '00000000-0000-0000-0000-000000000000' AND ParentSystemItemId is NULL) OR (ParentSystemItemId = @ParentSystemItemId)) and [MappedSystemId] = mp.SourceDataStandardMappedSystemId
        UNION all
...
SELECT EntityName as ItemName,
        ...
        COUNT(*) as Total,
		SUM(case when WorkflowStatusTypeId = 0 then 1 else 0 end) Unmapped,
