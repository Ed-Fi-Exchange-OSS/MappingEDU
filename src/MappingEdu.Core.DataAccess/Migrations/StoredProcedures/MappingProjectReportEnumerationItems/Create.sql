...
CREATE PROCEDURE [dbo].[MappingProjectReportEnumerationItems]
		...
		FROM result si'
			+
			    CASE WHEN @ColumnName is not NULL THEN ' LEFT JOIN #CustomDetails cd on si.SystemItemId = cd.SystemItemId '
				    ELSE '' END
		    +
		'JOIN SystemEnumerationItem sei on si.SystemItemId = sei.SystemItemId
		WHERE si.ItemTypeId = 5
		ORDER BY ElementGroup, ItemName;'
