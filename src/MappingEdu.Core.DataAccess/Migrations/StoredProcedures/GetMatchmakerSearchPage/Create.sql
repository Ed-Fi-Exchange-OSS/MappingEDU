...
CREATE PROCEDURE [dbo].[GetMatchmakerSearchPage]
...
WHERE i2.IsActive = 1 AND i2.ItemTypeId != 1 AND i2.MappedSystemId = @MappedSystemId AND
	((result.Level = 0 AND (@EntityIds = 'All' OR @EntityIds like '%;'+cast(i2.SystemItemId as varchar(50))+';%')) OR result.Level != 0))
...
WHERE (@ItemTypeIds = 'All' OR @ItemTypeIds like '%;'+cast(si.ItemTypeId as varchar(20))+';%') AND
