...
CREATE PROCEDURE [dbo].[MappingProjectReportTargetElementMappings]
...
SET @DynamicPivotQuery4 = N'
		...
		SELECT tsi.DomainItemPath,
			    ...
			    si.DomainItemPath as SourceDomainItemPath,
			    tsi.MappingMethodTypeId,
			    tsi.WorkflowStatusTypeId,
			    tsi.BusinessLogic,
			    tsi.OmissionReason,
			    tsi.CreatedBy,
			    tsi.CreateDate,
...
SET @DynamicPivotQuery5 = N'
		FROM #TEMP tsi
		LEFT JOIN result2 si on tsi.SourceSystemItemId = si.SystemItemId
