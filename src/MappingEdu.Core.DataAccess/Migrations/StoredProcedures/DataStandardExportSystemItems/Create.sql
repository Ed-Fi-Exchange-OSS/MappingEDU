...
CREATE PROCEDURE [dbo].[DataStandardExportSystemItems]
...
SET @DynamicPivotQuery4 = N'
		...
		WHERE si.MappedSystemId = ''' + CONVERT(nvarchar(50), @DataStandardId) + ''' AND 
				eg.IsActive = 1 AND si.IsActive = 1

		ORDER BY LEFT(si.DomainItemPath, LEN(si.DomainItemPath) - CASE WHEN (si.ItemTypeId = 4 OR si.ItemTypeId = 5) THEN LEN(si.ItemName) ELSE 0 END), ItemName;'
