...
// ****************************************************************************
// Module app.directives.mentio
//

var m = angular.module('app.directives.mentio', []);


// ****************************************************************************
// Directive mentio
//
// Modified for MappingEDU to allow chaining of elements in the business logic
// To have work SystemItems set isBusinessLogic to true
// i.e. mentio-is-business-logic="true"
//
// Downloaded from https://github.com/jeff-collins/ment.io


m.directive('mentio', ['mentioUtil', '$document', '$compile', '$log', '$timeout',
    function (mentioUtil, $document, $compile, $log, $timeout) {
        return {
            ...
            scope: {
                ...
                ngModel: '=',
                isBusinessLogic: '=mentioIsBusinessLogic' //Added For MappingEDU
            },
            controller: ["$scope", "$timeout", "$attrs", function ($scope, $timeout, $attrs) {
                ...
                $scope.replaceText = function (text, hasTrailingSpace) {
                    ...
                    mentioUtil.replaceTriggerText($scope.context(), $scope.targetElement, $scope.targetElementPath,
                        ...
                        hasTrailingSpace, $scope.suppressTrailingSpace, $scope.isBusinessLogic);
                ...
                $document.on(
                    'keydown keypress paste', function (event) {
                        ...
                        var typedEqualsFirstItem = false;

                        //Specific for MappingEDU BusinessLogic
                        if ($scope.isBusinessLogic && $scope.items && $scope.items[0]) {
                            if (activeMenuScope && activeMenuScope.typedTerm) {
                                if (activeMenuScope.typedTerm.toLowerCase() === $scope.items[0].label.toLowerCase())
                                    typedEqualsFirstItem = true;
                            }
                        }

                        if (activeMenuScope) {
                            if (event.which === 9 || event.which === 13 || ($scope.isBusinessLogic && typedEqualsFirstItem && event.which === 190)) {
            ...
            link: function (scope, element, attrs) {
                ...
                if (attrs.mentioItems) {
                    ...
                    if (scope.isBusinessLogic)
                        html += ' mentio-is-business-logic="true"';
                ...
                scope.$watch(
                    ...
                    function (newValue) {
                        ...
                        var mentionInfo = mentioUtil.getTriggerInfo(scope.context(), scope.triggerCharSet,
                            scope.requireLeadingSpace, isActive, null, scope.isBusinessLogic);
    ...
    .directive('mentioMenu', ['settings', 'mentioUtil', '$rootScope', '$log', '$window', '$document',
        function (settings: ISystemSettings, mentioUtil, $rootScope, $log, $window, $document) {
            return {
                restrict: 'E',
                scope: {
                    search: '&mentioSearch',
                    ...
                    isBusinessLogic: '=mentioIsBusinessLogic'
                },
                templateUrl: function (tElement, tAttrs) {
                    return tAttrs.mentioTemplateUrl !== undefined ? tAttrs.mentioTemplateUrl : `${settings.directiveBaseUri}/Mentio/mentio-menu.tpl.html`;
                },
                        ...
                        'resize', function () {
                            if (scope.isVisible()) {
                                ...
                                mentioUtil.popUnderMention(scope.parentMentio.context(),
                                    triggerCharSet, element, scope.requireLeadingSpace, scope.isBusinessLogic);
                            }
                        }
                    );
    ...
    .directive('mentioMenuItem', function () {
        return {
            ...
            require: '^mentioMenu',
            link: function (scope: any, element, attrs, controller: any) {
...
m.factory('mentioUtil', ["$window", "$location", "$anchorScroll", "$timeout", function ($window, $location, $anchorScroll, $timeout) {

    // public
    function popUnderMention(ctx, triggerCharSet, selectionEl, requireLeadingSpace, isBusinessLogic) {
        var coordinates;
        var mentionInfo = getTriggerInfo(ctx, triggerCharSet, requireLeadingSpace, false, null, isBusinessLogic);
    ...
    function selectedElementIsTextAreaOrInput(ctx?: any) {
    ...
    function replaceTriggerText(ctx, targetElement, path, offset, triggerCharSet,
        text, requireLeadingSpace, hasTrailingSpace, suppressTrailingSpace, isBusinessLogic) {
        resetSelection(ctx, targetElement, path, offset);

        var mentionInfo = getTriggerInfo(ctx, triggerCharSet, requireLeadingSpace, true, hasTrailingSpace, isBusinessLogic);
    ...
    // public
    function getTriggerInfo(ctx, triggerCharSet, requireLeadingSpace, menuAlreadyActive, hasTrailingSpace, isBusinessLogic) {
        ...
        if (effectiveRange !== undefined && effectiveRange !== null) {
            ...
            if (mostRecentTriggerCharPos >= 0 &&
                ...
                if (!leadingSpace && (menuAlreadyActive ||
                    (isBusinessLogic && !(/[\]]/g.test(currentTriggerSnippet)) && !(/[\.]/g.test(currentTriggerSnippet)) && !(/[\]]/g.test(currentTriggerSnippet))) ||
                    (!isBusinessLogic && !(/[\xA0\s]/g.test(currentTriggerSnippet)))
                )) {
